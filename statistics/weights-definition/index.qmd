...
when you have really low response rates, the probability sample might not differ much from a non-probability convience sample. Moreover, many weighting techniques come into play that could make the non-probability produce accurate and reliable estimates. The magic sparks on which variable you choose to weight your sample. For the Literary Digest example, Lohr and Brick (2017) finds that if votes cast in 1932 was used to weight the results, the poll would have predicted a majority of electoral votes for Roosevelt in 1936.

positive example of statistical adjustments

opt-in samples or other convience samples, use weighting to correct for lack of representativeness, Gelman's xbox project, using opt-in poll from the Xbox gaming platform during the month or so before the 2012 presidential election - Gelman uses party ID to reweight the data, determining which variables to incorporate to weight on is tricky and can be controversial, Gelman uses party ID to reweight the xbox data because this is not fixed and cannot be pulled from census like other demographic characteristics.

The Literary Digest example

## Weighting does not solve everything

Weighting is not a solution to every problem.
